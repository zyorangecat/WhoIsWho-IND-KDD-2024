...
from utils import  INDDataSetForLLama, DataCollatorForLLama
...
def main():
    ...
    model = AutoModelForCausalLM.from_pretrained(model_args.model_name_or_path, **model_kwargs).cuda()
    ...
    train_dataset = INDDataSetForLLama(
        (train_data,pub_data),
        tokenizer,
    ...
    data_collator = DataCollatorForLLama(
        tokenizer,
        max_seq_len=data_args.max_source_length + data_args.max_target_length + 4
    )

    print('>>>>>>>>>>>>>>>>>>>>>>>>>>>>> ',vars(data_collator))
